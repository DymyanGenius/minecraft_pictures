...
execute if score @s bv_paid matches 1.. run tag @e[tag=bv_offer_bv_stick,distance=..8,limit=1,sort=nearest] remove bv_offer_bv_stick
execute if score @s bv_paid matches 1.. if entity @e[tag=bv_random_recipe_seller,distance=..8,limit=1,sort=nearest] run kill @e[tag=bv_random_recipe_seller,distance=..8,limit=1,sort=nearest]
