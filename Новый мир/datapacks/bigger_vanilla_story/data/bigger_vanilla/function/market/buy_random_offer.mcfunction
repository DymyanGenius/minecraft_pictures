execute unless entity @e[tag=bv_random_recipe_seller,distance=..8] unless entity @e[tag=bv_recipe_villager,distance=..8] run tellraw @s {"text":"Рядом нет торговца рецептов. Найди обычного жителя-чертежника или призови случайного торговца через меню.","color":"red"}
